...
function withTimeout(promise, timeout = DEFAULT_TIMEOUT) {
  return Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(() => reject(new ApiError('Request timeout', API_ERROR_TYPES.TIMEOUT, 408)), timeout)
    )
  ])
}
...
async function handleResponse(response) {
  ...
  if (!response.ok) {
    ...
    let message = data?.message || `HTTP ${response.status}: ${response.statusText}`
...
export const apiService = {
  ...
  volunteers: {
    ...
    updateStatus: (id, data) => api.put(endpoints.volunteers.updateStatus(id), data),
  ...
  contact: {
    ...
    updateStatus: (id, data) => api.put(endpoints.contact.updateStatus(id), data),
  ...
  impact: {
    ...
    getTrends: (months = 12) => api.get(`${endpoints.impact.trends}?months=${months}`)
  }
}
